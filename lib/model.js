...
class Model {
  ...
  static async findManyBy(field, val) {
    const query = {
      text: `SELECT ${this.__columns().join(', ')} FROM ${this.prototype.__tableName} WHERE ${field} = $1`,
      values: [val]
    }
    return await db.query(query)
      .then(res => {
        ...
        for (let row of res.rows) {
          objects.push(new this.prototype.constructor(row))
        }
        return objects
      })
      .catch(this.__catchError)
  ...
  static async count({ where = [] } = {}) {
    const query = {
      text: `SELECT COUNT(*) AS count FROM ${this.prototype.__tableName}`,
      values: []
    }
    if (where.length >= 2) {
      query.text = `${query.text} WHERE ${where[0]}`
      query.values = where[1]
    }
    return await db
  ...
  async destroy() {
    const query = {
      text: `DELETE FROM ${this.__proto__.__tableName} WHERE id = $1`,
      values: [this.id]
    }
    return await db.query(query)
  }
